...
const steps = [
    { id: 1, title: 'Business Profile', description: 'Your scale & numbers' },
    { id: 2, title: 'Growth Goals', description: 'Vision & contact' },
];
                        ...
                        <div className="space-y-4 md:space-y-5 py-2">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input id="business-type" type="text" autoFocus className={inputClass('businessType')} value={formData.businessType} onChange={e => setFormData({ ...formData, businessType: e.target.value })} placeholder="e.g. Retail, SaaS, Agency" />
                                    {errors.businessType && <p className="text-red-400 text-xs mt-1 ml-1">{errors.businessType}</p>}
                                </div>
                                <div>
                                    <label className={labelClass}><MapPin size={12} className="text-purple-400" /> City / Location</label>
                                    <input id="city" type="text" className={inputClass('city')} value={formData.city} onChange={e => setFormData({ ...formData, city: e.target.value })} placeholder="e.g. Mumbai" />
                            ...
                            <div>
                                ...
                                <input id="customers" type="text" className={inputClass('customers')} value={formData.customers} onChange={e => setFormData({ ...formData, customers: e.target.value })} placeholder="e.g. 150 B2B clients, or 5000 D2C orders" />
                                {errors.customers && <p className="text-red-400 text-xs mt-1 ml-1">{errors.customers}</p>}
                            </div>

                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div>
                                    <label className={labelClass}><IndianRupee size={12} className="text-purple-400" /> Current Revenue</label>
                                    <input id="revenue" type="text" className={inputClass('revenue')} value={formData.revenue} onChange={e => setFormData({ ...formData, revenue: e.target.value })} placeholder="e.g. ₹5 Lakh/mo" />
                                    {errors.revenue && <p className="text-red-400 text-xs mt-1 ml-1">{errors.revenue}</p>}
                                </div>
                                <div>
                                    <label className={labelClass}><Wallet size={12} className="text-purple-400" /> Current Expense</label>
                                    <input id="expense" type="text" className={inputClass('expense')} value={formData.expense} onChange={e => setFormData({ ...formData, expense: e.target.value })} placeholder="e.g. ₹3 Lakh/mo" />
                                    {errors.expense && <p className="text-red-400 text-xs mt-1 ml-1">{errors.expense}</p>}
                                </div>
                                <div>
                                    <label className={labelClass}><Target size={12} className="text-purple-400" /> Profit After Tax</label>
                                    <input id="profit" type="text" className={inputClass('profit')} value={formData.profit} onChange={e => setFormData({ ...formData, profit: e.target.value })} placeholder="e.g. ₹1.5 Lakh/mo" />
                                    {errors.profit && <p className="text-red-400 text-xs mt-1 ml-1">{errors.profit}</p>}
                                </div>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="space-y-4 py-2">
                             <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                <div>
                                    ...
                                    <input id="full-name" type="text" autoFocus className={inputClass('fullName')} value={formData.fullName} onChange={e => setFormData({ ...formData, fullName: e.target.value })} placeholder="John Doe" />
                                    {errors.fullName && <p className="text-red-400 text-xs mt-1 ml-1">{errors.fullName}</p>}
                                </div>
                                <div>
                                    <label className={labelClass}><Mail size={12} className="text-purple-400" /> Email</label>
                                    <input id="email" type="email" className={inputClass('email')} value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} placeholder="john@example.com" />
                                    {errors.email && <p className="text-red-400 text-xs mt-1 ml-1">{errors.email}</p>}
                                </div>
                                <div>
                                    <label className={labelClass}><Phone size={12} className="text-purple-400" /> Phone</label>
                                    <input id="phone" type="tel" className={inputClass('phone')} value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} placeholder="+91 99999 99999" />
                                    {errors.phone && <p className="text-red-400 text-xs mt-1 ml-1">{errors.phone}</p>}
                                </div>
                            </div>

                            <div>
                                <label className={labelClass}><Rocket size={12} className="text-purple-400" /> Short Term Goal</label>
                                <textarea id="short-term-goal" rows={2} className={`${inputClass('shortTermGoal')} resize-none`} value={formData.shortTermGoal} onChange={e => setFormData({ ...formData, shortTermGoal: e.target.value })} placeholder="Your immediate priority..." />
                                {errors.shortTermGoal && <p className="text-red-400 text-xs mt-1 ml-1">{errors.shortTermGoal}</p>}
                            </div>

                            <div>
                                <label className={labelClass}><Target size={12} className="text-purple-400" /> Long Term Goal</label>
                                <textarea id="long-term-goal" rows={2} className={`${inputClass('longTermGoal')} resize-none`} value={formData.longTermGoal} onChange={e => setFormData({ ...formData, longTermGoal: e.target.value })} placeholder="Where do you see the business ultimately?" />
                            ...
                            {submitError && (
                                <div className="bg-red-500/10 border border-red-500/30 rounded-2xl px-4 py-3 text-[10px] md:text-xs text-red-400 leading-relaxed mt-2">
